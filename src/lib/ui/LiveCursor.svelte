...
<script lang="ts">
  ...
  export let user: ws_protocol.WsUser;
  export let showName = false;

  let hovering = false;
  let lastMove = Date.now();

  let lastCursor: ws_protocol.WsCursor | null = null;
  let time = Date.now();
  $: if (
    !lastCursor ||
    (user.cursor &&
      (lastCursor?.x !== user.cursor.x || lastCursor?.y != user.cursor.y))
  ) {
    lastCursor = user.cursor ? new ws_protocol.WsCursor(user.cursor) : null;
    lastMove = Date.now();
    setTimeout(() => {
      time = Date.now();
    }, 1600);
